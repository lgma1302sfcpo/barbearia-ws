...
function Appointments({ items, totals, selectedDate, post, remove, monthly }: { items: Appointment[]; totals: ReturnType<typeof dayTotals>; selectedDate: string; post: (payload: Record<string, unknown>, success: string) => Promise<boolean>; remove: (entity: string, id: number) => Promise<void>; monthly: ReturnType<typeof monthTotals> }) {
  ...
  return <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_380px]"><DataCard title={`Atendimentos de ${displayDate(selectedDate)}`} subtitle="Tabelas de Flávio e Fernando reunidas" count={items.length}>{items.length ? <table className="data-table"><thead><tr><th>Profissional</th><th>Serviço / cliente</th><th>Pagamento</th><th>Horário</th><th className="text-right">Valor</th><th /></tr></thead><tbody>{items.map((item) => <tr key={item.id}><td><Person name={item.professional} /></td><td><strong>{item.service}</strong><small>{item.client}</small></td><td>{item.payment}</td><td>{item.time}</td><td className="text-right font-bold">{money.format(item.amount_cents / 100)}</td><td className="w-10"><Delete onClick={() => void remove('appointment', item.id)} /></td></tr>)}</tbody></table> : <Empty text="Nenhum atendimento neste dia." />}</DataCard><aside className="space-y-5"><form id="entry-form" onSubmit={submit} className="form-card"><FormTitle title="Novo atendimento" text="60% e 40% são calculados na hora." /><Field label="Profissional"><NativeSelect className="w-full" value={professional} onChange={(e) => setProfessional(e.target.value as 'Flávio' | 'Fernando')}><NativeSelectOption>Flávio</NativeSelectOption><NativeSelectOption>Fernando</NativeSelectOption></NativeSelect></Field><Field label="Serviço"><Input value={service} onChange={(e) => setService(e.target.value)} required /></Field><Field label="Cliente"><Input value={client} onChange={(e) => setClient(e.target.value)} /></Field><div className="grid grid-cols-2 gap-3"><Field label="Pagamento"><Payment value={payment} onChange={setPayment} /></Field><Field label="Horário"><Input type="time" value={time} onChange={(e) => setTime(e.target.value)} required /></Field></div><Field label="Valor"><Input inputMode="decimal" value={amount} onChange={(e) => setAmount(e.target.value)} required /></Field><Split cents={toCents(amount)} /><Submit>Adicionar atendimento</Submit></form><ProfessionalCard daily={totals} monthly={monthly} /></aside></div>;
}
...
function Submit({ children, disabled = false }: { children: ReactNode; disabled?: boolean }) { return <Button disabled={disabled} className="mt-1 h-11 w-full bg-[#172b36] font-bold text-white hover:bg-[#274553]"><Plus />{children}</Button>; }
function Person({ name }: { name: string }) { return <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-bold ${name === 'Flávio' ? 'bg-[#daece7] text-[#266657]' : 'bg-[#e7e8f5] text-[#474b84]'}`}>{name}</span>; }
...
function ProfessionalCard({ daily, monthly }: { daily: ReturnType<typeof dayTotals>; monthly: ReturnType<typeof monthTotals> }) { return <section className="rounded-2xl bg-[#172b36] p-5 text-white shadow-sm"><p className="mb-4 text-xs font-bold uppercase tracking-[.14em] text-white/55">Por profissional no dia</p><PersonSummary name="Flávio" total={daily.flavio} /><div className="my-4 h-px bg-white/10" /><PersonSummary name="Fernando" total={daily.fernando} /><div className="mt-5 rounded-xl bg-white/7 p-3 text-xs text-white/60">No mês: Flávio {money.format(monthly.flavio / 100)} · Fernando {money.format(monthly.fernando / 100)}</div></section>; }
