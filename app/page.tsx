...
function Appointments({ items, totals, selectedDate, post, remove, monthly }: { items: Appointment[]; totals: ReturnType<typeof dayTotals>; selectedDate: string; post: (payload: Record<string, unknown>, success: string) => Promise<boolean>; remove: (entity: string, id: number) => Promise<void>; monthly: ReturnType<typeof monthTotals> }) {
  ...
  return <div className="space-y-5"><div className="grid items-start gap-5 lg:grid-cols-2"><ProfessionalPanel name="Flávio" date={selectedDate} items={flavioItems} total={totals.flavio} remove={remove} /><ProfessionalPanel name="Fernando" date={selectedDate} items={fernandoItems} total={totals.fernando} remove={remove} /></div><div className="grid items-start gap-5 lg:grid-cols-2"><form id="entry-form" onSubmit={submit} className="form-card"><FormTitle title="Novo atendimento" text="Escolha o profissional. O lançamento aparece no lado correto." /><Field label="Profissional"><NativeSelect className="w-full" value={professional} onChange={(e) => setProfessional(e.target.value as 'Flávio' | 'Fernando')}><NativeSelectOption>Flávio</NativeSelectOption><NativeSelectOption>Fernando</NativeSelectOption></NativeSelect></Field><div className="grid gap-3 sm:grid-cols-2"><Field label="Serviço"><Input value={service} onChange={(e) => setService(e.target.value)} required /></Field><Field label="Cliente"><Input value={client} onChange={(e) => setClient(e.target.value)} /></Field></div><div className="grid grid-cols-2 gap-3"><Field label="Pagamento"><Payment value={payment} onChange={setPayment} /></Field><Field label="Horário"><Input type="time" value={time} onChange={(e) => setTime(e.target.value)} required /></Field></div><Field label="Valor"><Input inputMode="decimal" value={amount} onChange={(e) => setAmount(e.target.value)} required /></Field><Split cents={toCents(amount)} /><Submit>Adicionar atendimento</Submit></form><ProfessionalCard daily={totals} monthly={monthly} /></div></div>;
}
