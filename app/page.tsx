...
const expectedServicePrice = (service: string) => SERVICE_OPTIONS.find((item) => item.name.toLocaleLowerCase('pt-BR') === service.trim().toLocaleLowerCase('pt-BR'))?.cents ?? null;
const hasPaymentOrPriceIssue = (item: Appointment) => item.payment === 'Não pagou' || (expectedServicePrice(item.service) !== null && item.amount_cents !== expectedServicePrice(item.service));
...
export default function Home() {
  ...
  function openEntryForm() {
    ...
    window.requestAnimationFrame(() => window.requestAnimationFrame(() => {
      if (isMobile) document.getElementById('entry-form')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    ...
    <main className="min-h-screen bg-background text-foreground">
      <header className="z-20 border-b border-white/10 bg-[#172b36] text-white shadow-sm sm:sticky sm:top-0">
        <div className="mx-auto flex max-w-[1480px] flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-8">
          ...
          <div className="order-2 flex gap-2 sm:order-3"><Button variant="ghost" className="h-9 text-white/70 hover:bg-white/10 hover:text-white" onClick={() => void logout()}><LogOut /> Bloquear</Button><Button className="h-9 bg-[#f2a24a] px-3 font-bold text-[#172b36] hover:bg-[#ffb65f]" onClick={openEntryForm}><Plus /> Lançar</Button></div>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-[1480px] px-4 py-6 sm:px-8">
        <section className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          ...
          <div className="flex w-full flex-wrap items-center gap-2 sm:w-auto"><Button variant="outline" onClick={exportCsv}><Download /> Exportar CSV</Button><div className="flex w-full min-w-0 items-center gap-1 rounded-xl border bg-card p-1 shadow-sm sm:w-auto"><Button className="shrink-0" variant="ghost" size="icon" onClick={() => changeMonth(-1)} aria-label="Mês anterior"><ChevronLeft /></Button><Input className="mobile-date-input h-10 min-w-0 flex-1 basis-0 border-0 text-center text-base font-semibold shadow-none focus-visible:ring-0 sm:h-8 sm:w-[150px] sm:flex-none sm:text-sm" type="date" value={selectedDate} onChange={(e) => { setSelectedDate(e.target.value); setMonth(e.target.value.slice(0, 7)); }} /><Button className="shrink-0" variant="ghost" size="icon" onClick={() => changeMonth(1)} aria-label="Próximo mês"><ChevronRight /></Button></div></div>
        </section>

        {notice && <div className="fixed right-4 top-20 z-40 max-w-sm rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-800 shadow-lg sm:right-8">{notice}</div>}
...
function Appointments({ items, totals, selectedDate, save, remove, monthly, recentlyAddedId }: { items: Appointment[]; totals: ReturnType<typeof dayTotals>; selectedDate: string; save: (payload: Record<string, unknown>, success: string, method?: 'POST' | 'PATCH') => Promise<boolean>; remove: (entity: string, id: number) => Promise<void>; monthly: ReturnType<typeof monthTotals>; recentlyAddedId: number | null }) {
  ...
  return <div className="space-y-4"><div className="grid items-start gap-4 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]"><div className="professional-carousel"><ProfessionalPanel name="Flávio" date={selectedDate} items={flavioItems} total={totals.flavio} remove={remove} edit={edit} recentlyAddedId={recentlyAddedId} /><ProfessionalPanel name="Fernando" date={selectedDate} items={fernandoItems} total={totals.fernando} remove={remove} edit={edit} recentlyAddedId={recentlyAddedId} /></div><form id="entry-form" onSubmit={submit} className="form-card entry-form !gap-2.5 !p-4"><div className="flex items-start justify-between gap-3"><FormTitle title={editingId ? 'Editar atendimento' : 'Novo atendimento'} text={editingId ? 'Altere os dados e salve.' : 'O lançamento aparece no lado correto.'} />{editingId && <Button type="button" variant="ghost" size="icon-sm" onClick={resetForm} aria-label="Cancelar edição"><X /></Button>}</div><Field label="Profissional"><NativeSelect className="w-full" value={professional} onChange={(e) => setProfessional(e.target.value as '' | 'Flávio' | 'Fernando')} required><NativeSelectOption value="" disabled>Selecione uma opção</NativeSelectOption><NativeSelectOption>Flávio</NativeSelectOption><NativeSelectOption>Fernando</NativeSelectOption></NativeSelect></Field><div className="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-1 2xl:grid-cols-2"><Field label="Serviço"><NativeSelect className="w-full" value={service} onChange={(e) => selectService(e.target.value)} required><NativeSelectOption value="" disabled>Selecione uma opção</NativeSelectOption>{service && expectedServicePrice(service) === null && <NativeSelectOption value={service}>{service} (antigo)</NativeSelectOption>}{SERVICE_OPTIONS.map((item) => <NativeSelectOption key={item.name} value={item.name}>{item.name} · {money.format(item.cents / 100)}</NativeSelectOption>)}</NativeSelect></Field><Field label="Cliente"><Input value={client} onChange={(e) => setClient(e.target.value)} /></Field></div><div className="grid grid-cols-2 gap-2.5"><Field label="Pagamento"><Payment value={payment} onChange={setPayment} /></Field><Field label="Horário"><Input type="time" value={time} onChange={(e) => setTime(e.target.value)} required /></Field></div><Field label="Valor"><Input inputMode="decimal" value={amount} onChange={(e) => setAmount(e.target.value)} required /></Field><Split cents={toCents(amount)} /><Submit>{editingId ? 'Salvar alterações' : 'Adicionar atendimento'}</Submit></form></div><ProfessionalCard daily={totals} monthly={monthly} /></div>;
}
...
function Beverages({ products, sales, selectedDate, post, remove }: { products: Product[]; sales: BeverageSale[]; selectedDate: string; post: (payload: Record<string, unknown>, success: string) => Promise<boolean>; remove: (entity: string, id: number) => Promise<void> }) {
  ...
  async function sale(event: FormEvent) { event.preventDefault(); if (await post({ entity: 'beverageSale', date: selectedDate, productId: Number(productId), quantity: Number(quantity), client }, 'Venda de bebida adicionada.')) { setProductId(''); setQuantity('1'); } }
...
function Payment({ value, onChange }: { value: string; onChange: (value: string) => void }) { return <NativeSelect className="w-full" value={value} onChange={(e) => onChange(e.target.value)} required><NativeSelectOption value="" disabled>Selecione uma opção</NativeSelectOption><NativeSelectOption>QR/CODE</NativeSelectOption><NativeSelectOption>Débito</NativeSelectOption><NativeSelectOption>Crédito</NativeSelectOption><NativeSelectOption>Dinheiro</NativeSelectOption><NativeSelectOption>Não pagou</NativeSelectOption></NativeSelect>; }
